...
session_start();
	if(isset($_POST['wyloguj_user'])){
		unset($_SESSION['zalogowany_user']);
		session_destroy();
		header('Location: http://serwer1686836.home.pl/kucharski/z7/');
	}
	else {
		...
		$klient = $_SESSION['zalogowany_user'];
...
//$file = $_REQUEST['file'] ?: "./users/benek/";
//$file = $_REQUEST['file'] ?: '.';
$file = $_REQUEST['file'] ?: "./users/".$klient;
...
$MAX_UPLOAD_SIZE = min(asBytes(ini_get('post_max_size')), asBytes(ini_get('upload_max_filesize')));
		...
		$result = mysqli_query($link, "SELECT logi.data, logi.czy_udane FROM logi,user  WHERE user.id=logi.id_usera AND user.login='$klient' ORDER BY logi.id DESC LIMIT 2;");
		while($r = mysqli_fetch_assoc($result)) {
			$data_log = $r['data'];
			$udane = $r['czy_udane'];
		}
		if($udane==0){
		echo "Ostatnio nastapilo bledne logowanie dnia: ".$data_log; echo "<br>";;
		}
		echo "Panel klienta"; echo "<br>";
		echo "Zalogowany jako: ".$klient; echo "<br>";		
		echo "<form method='post'>";
		echo "<button type='submit' name='wyloguj_user' value='Wylogowanie'>Wyloguj</button><br>";
		echo "</form>";	
?>
<!DOCTYPE html>
<html><head>
